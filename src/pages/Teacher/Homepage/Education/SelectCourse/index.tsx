 ...
 * @LastEditTime: 2021-12-02 16:18:27
...
import { Button, ConfigProvider, DatePicker, Divider, List, message, Modal, Radio, Tag } from 'antd';
...
const Study = () => {
  ...
  return (
    <div className={styles.selectedPage}>
      <GoBack title="我要选课" teacher onclick="/teacher/home?index=education" />
      <div className={styles.selectedCard}>
        <div className={styles.selectedCon}>
          <div className={styles.searchVal}>
            ...
            <span onClick={(e) => handleSearch(e, 'date')}>{moment(day).format('MM月DD日') || '今天'}</span>
          </div>
          <div className='searchWrapper' ref={divRef} onClick={(e) => {
            ...
            <div>
              ...
              <div className='gradeWrapper'>
                <ul>
                  <li className={NjId === undefined ? styles.active : ''} key={undefined} onClick={() => {
                    setNjId(undefined);
                    setNjVal('全年级');
                    divRef!.current!.style.display = 'none';
                  }}>
              ...
              <div className='typeWrapper'>
                <ul>
                  <li className={LXId === undefined ? styles.active : ''} key={undefined} onClick={() => {
                    setLXId(undefined);
                    setLXVal('全部课程');
                    divRef!.current!.style.display = 'none';
                  }}>
                  ...
                  {kclxOptions && kclxOptions?.map((item: any) => {
                    return <li
          ...
          renderItem={(item: any) => {
            ...
            return (
              <List.Item
                ...
                actions={[<input name="Fruit" type="radio" value={`${item.id}+${item.BMKSSJ}`} onClick={() => {
                  setCurrent(item);
                }} />]}
              >
                <List.Item.Meta
                  title={`${item?.KHKCSJ?.KCMC}【${item?.BJMC}】`}
                  description={`${XXSJPZ?.KSSJ?.substring?.(0, 5)} - ${XXSJPZ?.JSSJ?.substring?.(0, 5)} | ${FJSJ.FJMC}`}
                />
      ...
      {current ? <div className={styles.applyWrapper} onClick={(e) => {
        if (e.target === e.currentTarget) {
          setCurrent(undefined);
          setMainTeacher(undefined);
          setSubTeacher([]);
        ...
        <div>
          <div className={styles.title}>
            {`${current?.KHKCSJ?.KCMC} - ${current?.BJMC}`}
          </div>
          <div className={styles.content}>
            <div className={styles.items}>
              ...
              <div>
                {mainTeacher ? <Tag closable={mainTeacher?.JZGJBSJ?.id === userId} onClose={() => {
                  setMainTeacher(undefined);
                }}>
                  {mainTeacher?.JZGJBSJ?.XM === '未知' && mainTeacher?.JZGJBSJ?.WechatUserId ? (
                    <WWOpenDataCom type="userName" openid={mainTeacher?.JZGJBSJ?.WechatUserId} />
                  ) : (
                    mainTeacher?.JZGJBSJ?.XM
                  )}
                </Tag> : <span className={styles.tipText}>暂无安排</span>}
              </div>
            </div>
            <div className={styles.items}>
              <span>
              ...
              <div>
                {subTeacher?.length ? <>{subTeacher.map((v: any) => {
                  return <Tag closable={v?.JZGJBSJ?.id === userId} onClose={() => {
                    const subs = subTeacher.filter((val: any) => val.JZGJBSJ?.id !== userId);
                    setSubTeacher(subs);
                  }}>
                    {v?.JZGJBSJ?.XM === '未知' && v?.JZGJBSJ?.WechatUserId ? (
                      <WWOpenDataCom type="userName" openid={v?.JZGJBSJ?.WechatUserId} />
                    ) : (
                      v?.JZGJBSJ?.XM
                    )}
                  </Tag>
            ...
            <div className={styles.items}>
              <div>
                <h3>
                  选择角色
                  <span style={{ display: mainTeacher ? 'block' : 'none' }} className={styles.tipText}>
                    已有主班教师
                  </span>
                </h3>
              </div>
              <Radio.Group >
                <Radio.Button onClick={(e) => handleChange(e)} disabled={mainTeacher && mainTeacher?.JZGJBSJ?.id !== userId} value="1">主班</Radio.Button>
                <Radio.Button onClick={(e) => handleChange(e)} value="0">副班</Radio.Button>
              </Radio.Group>
            </div>
          </div>
          <Button className={styles.submit} onClick={() => prevJudge()}>提交</Button>
        </div>
      </div> : ''}
    </div>
  );
