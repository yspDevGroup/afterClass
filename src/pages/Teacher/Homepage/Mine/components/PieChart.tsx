...
const CheckOnChart = (props: { data: ItemType[]; title?: string; subTitle?: string, cls?: string }) => {
  ...
  return (
    <div className={`${styles.chartWrapper} ${cls}`}>
      ...
      {subTitle ? <div style={{ overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap',fontSize: 12,color:'#888' }}>{subTitle}</div> : ''}
      <Pie {...pieConfig} />
    </div>
  );
};
