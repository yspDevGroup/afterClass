...
import { createKHXSDD, deleteKHXSDD, getKHXSDD, overdueKHXSDD, payKHXSDD } from '@/services/after-class/khxsdd';
...
const AgentRegistration = (props: {
  ...
  useEffect(() => {
    if (OrderId) {
      (
        async () => {
          if ((BmCurrent === 3 && BjDetails?.KHKCJCs?.length !== 0) || (BmCurrent === 2 && BjDetails?.KHKCJCs?.length === 0) || (Number(DDZFY) <= 0 && BmCurrent === 1) || (BjDetails?.KHKCJCs?.length === 0 && BmCurrent === 2) || (Number(BjDetails?.FY) <= 0 && BmCurrent === 2 && Number(JFTotalost) > 0)) {
  ...
  const next = async () => {
    if (typeof XZBXSId !== 'undefined') {
      ...
      if ((BjDetails?.KHKCJCs?.length === 0 && BmCurrent === 0) || (BjDetails?.KHKCJCs?.length !== 0 && BmCurrent === 1) || (Number(DDZFY) <= 0 && BmCurrent === 1) || (BjDetails?.KHKCJCs?.length === 0 && BmCurrent === 2) || (Number(BjDetails?.FY) <= 0 && BmCurrent === 2 && Number(JFTotalost) > 0)) {
  ...
  return <>
    <Modal
      ...
      <>
        ...
        {
          BmCurrent === 0 ? <div className={styles.stepsContent}>
              ...
              >
                {
                  XZBDatas?.map((value: any) => {
                    return <Option value={`${value?.id}`} key={value?.id} data-value={`${value?.NJSJ?.NJMC}${value?.BJ}`}>{`${value?.NJSJ?.NJMC}${value?.BJ}`}</Option>
                  })
                }
              </Select>
            </div>
                    ...
                    return (
                      <Option value={value?.XM + value?.XH} key={value?.id}>
                        <div style={{
                          ...
                          <span>{showWXName ? <WWOpenDataCom type="userName" openid={value?.WechatUserId} /> : value?.XM}</span>
                          <span>{value?.XH}</span>
                        </div>
        ...
        {
          BjDetails?.KHKCJCs?.length !== 0 && BmCurrent === 1 ? <><div className={styles.JF}>

            <div className={styles.checkbox}>
              <div><Checkbox onChange={onChanges}>选购教辅？</Checkbox></div>
              <div>费用总计：<span>￥{JFTotalost}</span> </div>
            </div>
            <div>
              <div>
                <div>名称</div>
              ...
              </div>
              {
                BjDetails?.KHKCJCs?.map((value: any) => {
                  return <div className={styles.JFXX}>
                    <div>{value?.JCMC}</div>
                    <div>￥{value?.JCFY}</div>
                  </div>
                })
          ...
          </div></> : <></>}
        {
          BjDetails && Number(DDZFY) <= 0 ? <></> : <>
            {(BjDetails?.KHKCJCs?.length === 0 && BmCurrent === 1) || (BjDetails?.KHKCJCs?.length !== 0 && BmCurrent === 2) ? <div className={styles.PaymentCode} >
              {
                Message === '您已经报名成功，请勿重复报名!' ?
                  <Result
                    title="您已经报名成功，请勿重复报名!"
                    extra={
                      <Button type="primary" key="console" onClick={handleCancel} className={styles.ZFCG}>
                        关闭
                      </Button>
                    }
                  /> : <></>
              }
              {
                PaymentCG === '待支付' && Message !== '您已经报名成功，请勿重复报名!' ?
                  <>
                    ...
                    />内，用微信扫码支付，逾期订单将自动取消</p>
                    <div className={styles.wraps}>

                      {
                        FkHref ? <div><QRCode className={styles.QRCodeQrCode} value={FkHref} size={140} /></div> : <></>
                      }
                    </div>  <Button type="primary" onClick={onClickFK} className={styles.ZFWC}>
                      支付完成
                    </Button></> : <></>
              }
              {
        ...
        {
          (Number(DDZFY) <= 0 && BmCurrent === 1 && Number(JFTotalost) <= 0) || (BjDetails?.KHKCJCs?.length !== 0 && BmCurrent === 3) || (BjDetails?.KHKCJCs?.length === 0 && BmCurrent === 2) || (Number(BjDetails?.FY) <= 0 && BmCurrent === 3 && Number(JFTotalost) > 0) ? <div className={styles.details} >
            {
              Message === '您已经报名成功，请勿重复报名!' ?
                <Result
                  title="您已经报名成功，请勿重复报名!"
                ...
                /> : <>  <div className={styles.wraps}>
                  <div>
                    <p><span>课程名称</span><span>{BjDetails?.KHKCSJ?.KCMC}</span> </p>
                    <p><span>上课班级</span><span>{BjDetails?.BJMC}</span> </p>
                    <p><span>上课时段</span><span>{BjDetails?.KKRQ}~{BjDetails?.JKRQ}</span> </p>
                    <p><span>上课地点</span><span>本校</span> </p>
                    <p><span>总课时</span><span>{BjDetails?.KSS}</span> </p>
                    <div className={styles.line} />
                    <p><span>行政班级</span> <span>{BJMC}</span></p>
                    <p><span>学生</span> <span>{XSMC}</span></p>
                  </div>
                  <div>
                    <p><span>订单编号</span><span>{OrderDetails?.DDBH}</span> </p>
                    <p><span>下单时间</span><span>{OrderDetails?.XDSJ}</span> </p>
                    <p><span>支付方式</span><span>{OrderDetails?.ZFFS}</span> </p>
                    <p><span>支付时间</span><span>{OrderDetails?.ZFSJ || '-'}</span> </p>
                    <div className={styles.line} />
                    <p><span>课程费用</span> <span>{BjDetails?.FY}</span></p>
                    <p><span>教辅费用</span> <span>{(Number(OrderDetails?.DDFY) - Number(BjDetails?.FY)).toFixed(2)}</span></p>
                    <p><span>实付：</span> <span>￥{OrderDetails?.DDFY}</span></p>
                  </div>
                </div>
