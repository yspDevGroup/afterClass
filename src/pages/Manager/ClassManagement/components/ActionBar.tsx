  ...
  switch (record.BJZT) {
    ...
    case '已开班':
      return (
        <>
          {
            (new Date(record?.BMJSSJ).getTime() >= new Date().getTime()) && (record?.xs_count > 0) && (record?.xs_count < record?.BJRS) ?
              <> <a onClick={() => {
                setVisible(true)
              }}>取消开班</a>
                <Modal
                ...
                >
                  <Form
                    ...
                    wrapperCol={{ span: 15 }}
                    form={form}
                    initialValues={{ MSG: `您所选的${record?.KHKCSJ?.KCMC}-${record?.BJMC}，由于报名人数不足，已关闭该班级；相关课程费用将全额原路返还，请知悉。` }}
