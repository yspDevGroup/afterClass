...
const ClassScheduling = () => {
		...
		const screenNJ = (dataSource5: any) => {
			...
			if (NJId) {
				newDataSource.forEach((item: any) => {
					...
					if (KHPKSJs?.length > 0) {
						item.KHPKSJs = KHPKSJs?.filter(
							(KHPKSJ: any) => KHPKSJ?.KHBJSJ?.BJSJs?.[0]?.NJSJ?.id === NJId,
						);
					}
				});
				return newDataSource;
		...
		const screenXZB = (dataSource6: any) => {
			...
			if (XZBId) {
				newDataSource.forEach((item: any) => {
					...
					if (KHPKSJs?.length > 0) {
						item.KHPKSJs = KHPKSJs?.filter(
							(KHPKSJ: any) => KHPKSJ?.KHBJSJ?.BJSJs?.[0]?.id === XZBId,
						);
					}
				});
				return newDataSource;
	...
	// 根据学年学期ID 校区id 获取学年课程名称数据，和班级名称数据， 获取当前学校学年的学期的场地排课情况
	useEffect(() => {
		// const bjId = getQueryString('courseId');
	...
	//     }
	// }, [teacher, cdmcValue, kcmcValue, bjmcValue, radioValue, XZBId, NJId]);
	useEffect(() => {
		// const bjId = getQueryString('courseId');
		if (screenOriSource && screenOriSource.length > 0) {
			// 筛选数据
				...
				{state === true ? (
						...
						<div className={styles.searchWrapper}>
							<SearchLayout>
								...
								<div>
									...
									<Select value={XZBId} allowClear placeholder="行政班名称" onChange={(value: string) => {
										setXZBId(value)
									}}>
                  ...
                  <div>
                    ...
                    />
						...
						{/* 场地排课情况 单选按钮组件 */}
						<div style={{ width: '100%', padding: '24px 0 0 24px', display: 'flex', justifyContent: "space-between", alignItems: 'center' }}>
							<div>
								<span>场地排课情况：</span>
								<span>
									<Radio.Group
