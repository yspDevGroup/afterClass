  ...
  const getData = async (XNXQId: string) => {
    ...
    if (result?.status === 'ok') {
      ...
      if (data) {
        ...
        defaultData.schoolNum = data.kcbm?.length ? [].map.call(data.kcbm, (item: any) => {
          return item.KCMC;
        }) : [];
        data.kcbm?.length && data.kcbm.forEach((item: any) => {
        ...
        defaultData.enrollNum = data.kcbm?.length ? [].map.call(data.kcbm, (item: any) => {
          return {
            school: item.KCMC,
            value: item.xs_count,
          };
