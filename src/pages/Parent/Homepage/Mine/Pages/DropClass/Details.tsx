...
const Details = (props: any) => {
  ...
  useEffect(() => {
    if (state.ZT === 0) {
      ...
      state?.KHXSTKs?.length !== 0 &&
      (state?.KHXSTKs?.[0].TKZT === 0 || state?.KHXSTKs?.[0].TKZT === 1)
    ) {
      setType('退款中');
    ...
    <MobileCon>
      ...
      <div className={styles.OrderDetails}>
        ...
        >
          {state?.XSFWBJ ? (
            <>
              <div className={styles.FWXX}>
                <ul>
                  <p className={styles.title}>{state?.XSFWBJ?.KHFWBJ?.FWMC}</p>
                  <li>
                    服务时段：{state?.XSFWBJ?.KHFWSJPZ?.KSRQ}~{state?.XSFWBJ?.KHFWSJPZ?.JSRQ}
                  </li>
                  <li>
                    学生：
              ...
              <div className={styles.KCZE}>
                ...
                {Type === '退款成功' ? (
                  <p>
                    <span>退款时间</span>
                    <span>{state?.KHXSTKs?.[0].TKSJ}</span>
                  </p>
                ) : (
                  <></>
                )}
                <p>
                  <span>服务总额</span>{' '}
                  <span>￥{Number(state?.XSFWBJ?.KHFWBJ?.FWFY).toFixed(2)}</span>
                </p>
                {Type === '退款成功' ? (
                  <p>
                    <span>退款金额</span>
                    <span>￥{Number(state?.KHXSTKs?.[0].TKJE).toFixed(2)}</span>
                  </p>
                ) : (
                  <></>
                )}
            ...
            <>
              <div className={styles.FWXX}>
                <ul>
                  ...
                  <li>服务类型：{state?.KHXXZZFW?.KHZZFW?.FWMC}</li>
                  <li>
          ...
          {Type === '退订失败' || Type === '退款被驳回' ? (
            <div className={styles.KCZE}>
              {state.ZT === 2 && state?.KHXSTKs?.length === 0 ? <p>退订说明：{state?.BZ}</p> : ''}
              {state?.KHXSTKs?.length !== 0 && state?.KHXSTKs?.[0].TKZT === '2' ? (
                <p>退款说明：{state?.KHXSTKs?.[0].BZ}</p>
              ) : (
                ''
              )}
            </div>
