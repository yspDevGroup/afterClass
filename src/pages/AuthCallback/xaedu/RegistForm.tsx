...
import { bindUser, checkCode, getUsers } from '@/services/after-class/xaedu';
...
const SuccessCon = () => {
  return (
    <>
      ...
      <CheckCircleOutlined
        style={{ fontSize: '3rem', color: 'limegreen', margin: '2rem 0 4rem' }}
      />
      <div>
        <Button
          type="primary"
...
type RegistFormProps = { show: boolean; username: string };

const RegistForm = ({ show, username }: RegistFormProps) => {
  // 按操作步骤控制显示信息
  const [showModel, setShowModel] = useState<
    'default' | 'mobileSuccess' | 'mobileError' | 'codeSended' | 'success'
  >('default');
  // 获取到的用户所属学校列表
    ...
    >
      {showModel === 'success' ? (
        ...
        <>
          ...
          {showModel === 'default' && (
            <p>我们未找到您在本系统中的账号信息，如果您是第一次登录，请先绑定用户信息。</p>
          )}
          ...
          <ProForm<{ phone: string }>
            ...
            onFinish={async (formData) => {
              ...
              const res = await getUsers({ phone: formData.phone, plat: 'school', username });
          ...
          {showModel === 'codeSended' && (
            <Button
              key="submit"
              type="primary"
              onClick={bindformRef?.current?.submit}
