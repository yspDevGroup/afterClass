          ...
          data?.cla === '无法排课' ? <div className={styles.NoPk}><p>超出学年学期</p><span>无法排课</span></div> :
            <div className="classCard">
              ...
              >
                ...
                {mode === 'see' ? (<>
                  {
                    bjmcValue?.length <= 1 ? <div className="teacher" style={{ height: 22 }}>
                      <span>{data?.fjmc}</span>
                    </div> : <></>
                  }</>
                ) : (
                ...
                return (
                  <tr key={Math.random()}>
                    {columns.map((item, itemKey) => {
                      ...
                      if (item.dataIndex === 'room' || item.dataIndex === 'course') {
                        ...
                        return (
                          <td
                            ...
                            style={{ width: item.width, textAlign: item.align, height: bjmcValue?.length > 1 ?  '48px':'78px'  }}
                          ...
                          >
                            {type === 'edit' ? (
                              ...
                              <div className="classCard" style={{ textAlign: item.align, display: 'contents' }}>
                      ...
                      return (
                        <td key={`${item.key}-${data.key}`} style={{ width: item.width, height: bjmcValue?.length > 1 ? '48px' : '78px' }}>
                          {
