...
const CourseList = (props: { data: any }) => {
  const { data } = props;
  return (
    <>
                ...
                >
                  <p>
                    ...
                    <span
                      style={{
                        color:
                          ClassStatus(value.start, value.end) === '待上课' ? '#15B628' : '#999',
                      }}
                    >
                      {value?.status || ClassStatus(value.start, value.end)}
                    </span>
                  </p>
                  <div className={styles.box}>
                    ...
                    <div>
                      ...
                      <p>
                        {' '}
                        <EnvironmentOutlined />
                        {value?.xq || '本校'} {value?.address ? <>| {value?.address}</> : <></>}
                      </p>
                      <p>
                        {' '}
                        <TeamOutlined />
                        {value?.BJMC} {value?.BJRS ? <> | 共{value?.BJRS}人</> : <></>}{' '}
                      </p>
                    </div>
                  </div>
                </div>
